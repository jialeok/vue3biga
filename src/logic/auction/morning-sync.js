...
// 它确实在写库，但【落库时刻曾一路退化】（market_metrics 首批行 created_at，北京时间）：
//     2026-09-09 → 09:25:19 ✅   2026-09-10 → 09:26:18 ⚠️   2026-09-11 → 09:29:56 ❌
// 根因在 worker 侧：步骤1 九次串行网络调用、今天数据缺失时 sleep 20s+40s、
// market_metrics 按「日期 × 字段形状」串行 upsert（实测单次 ≈7s）。
// 已于 2026-09-11 修复（P0 今天优先 + 全并行，见 workers/bidding-auto-fetch/logic/morning-workflow.js），
// 目标：今天的数据 09:25:1x 落库。
//
// 即便如此，前端仍必须有自愈，因为：
//   ① 页面若在 9:25 前打开，首屏那次 30 天窗口拉取时当天还没有任何行；
...
/**
 * 重试间隔。[LATENCY 2026-09-11] 原为 20s —— worker 已优化到 09:25:1x 落库，
 * 20s 的节拍意味着最坏要等到 09:25:2x 才被前端看到，与「9:26 前必须能操作」冲突。
 * 改为 10s：窗口内最多 18 次 × 3 请求 = 54 次，只发生在 09:24~09:27 这三分钟内。
 */
