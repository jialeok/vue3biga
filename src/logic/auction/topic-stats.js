...
//   - 「竞价高开」数量 = 竞价涨幅【> 0】的只数（= 0 平开、< 0、无数据都不计，2026-09-12 修正）。
...
export function buildTopicStatsMap(entries, opts) {
  ...
  groups.forEach(function(arr, topic) {
    ...
    arr.forEach(function(e) {
      ...
      //   原实现写成 `auc >= 0`，把平开（0%）也算成高开 → 数字虚高（用户反馈）。
      //   口径与右侧「收盘 N红M绿」严格对齐：> 0 计入、< 0 不计、= 0 两边都不计。
      if (auc !== null && auc > 0) highOpen++;

      // [CLOSE-COUNT 2026-09-11] 收盘红绿：只统计「拿得到收盘涨幅」的票；
