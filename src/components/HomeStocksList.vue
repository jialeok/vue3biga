<template>
  <!-- 股票列表 -->
  <div
    class="stock-list trading-day-element"
    :class="{ collapsed: isCollapsedMode }"
  >
    <div
    ...
    >
      <div class="stock-header">
        <div
          ...
          @click="onHeaderLeftClick($event, stock)"
        >
          <div class="stock-name">
            {{ stock.name }}
            <div class="tags-row tags-row-first">
              <span
                v-if="headerTags(stock).pos1"
                class="tag"
                :class="headerTags(stock).pos1.cls"
              >{{ headerTags(stock).pos1.text }}</span>
              <span
                v-else
                class="tag tag-placeholder"
              />
              <span
                v-if="headerTags(stock).pos2"
                class="tag"
                :class="headerTags(stock).pos2.cls"
              >{{ headerTags(stock).pos2.text }}</span>
              <span
                v-else
                class="tag tag-placeholder"
              />
              <span
                v-if="headerTags(stock).pos3"
                class="tag"
                :class="headerTags(stock).pos3.cls"
              >
                <svg
                  v-if="headerTags(stock).pos3.svg"
                  width="12"
                  height="12"
                  viewBox="0 0 24 24"
                  fill="#fbbf24"
            ...
            <div class="tags-row">
              <span
                v-for="(tag, idx) in secondTags(stock)"
                :key="idx"
                class="tag"
                :class="tag.cls"
              >{{ tag.text }}</span>
        ...
        <div
          ...
          @click="onHeaderRightClick($event, stock)"
        >
          <div
            class="close-rate-header"
            :class="{ up: closeDisplay(stock).isUp, down: closeDisplay(stock).isDown }"
          >
            <span
              v-if="closeHeaderBadge(stock)"
              class="bomb-badge"
              :style="closeHeaderBadge(stock).style"
            >{{ closeHeaderBadge(stock).text }}</span>
      ...
      <div
        class="stock-body"
        @click="onBodyClick($event, stock)"
        @dblclick="onEdit(stock.id)"
      >
        <div class="info-item">
          ...
          <div
            ...
            :style="{ color: stocksTurnoverDisplay(stock).color }"
          >
            {{ stocksTurnoverDisplay(stock).display }}
          </div>
        </div>
        <div class="info-item">
          <div class="info-label">
          ...
          <div
            class="info-value"
            :style="{ color: openDisplay(stock).color }"
          >
            {{ openDisplay(stock).display }}
          </div>
        </div>
        <div class="info-item">
          <div class="info-label">
          ...
          <div
            class="info-value"
            :style="{ color: adjustDisplay(stock).color }"
          >
            <span
              v-if="adjustDisplay(stock).symbol"
              :style="{ color: adjustDisplay(stock).symbolColor }"
            >{{ adjustDisplay(stock).text }}{{ adjustDisplay(stock).symbol }}</span>
            <template v-else>
              {{ adjustDisplay(stock).text }}
            </template>
          </div>
        </div>
        <div class="info-item">
        ...
        <div class="info-item">
          ...
          <div class="remark-value">
            <span
              v-if="remarkDisplay(stock).prefix"
              :style="{ color: remarkDisplay(stock).prefixColor }"
            >{{ remarkDisplay(stock).prefix }}</span>{{ remarkDisplay(stock).text || '-' }}
          </div>
        </div>
        <div class="info-item">
          <div class="info-label">
          ...
          <div
            class="info-value"
            :style="{ color: kbkDisplay(stock).color }"
          >
            {{ kbkDisplay(stock).display }}
          </div>
        </div>
        <div class="info-item">
          <div class="info-label">
      ...
      <div
        :id="'actions-' + stock.id"
        class="stock-actions"
        :class="{ expanded: expandedActionsId === stock.id }"
      >
        <button
          class="action-btn btn-edit"
          @click.stop="onEdit(stock.id)"
        >
          编辑
        </button>
        <button
          class="action-btn btn-copy"
          @click.stop="onCopyTomorrow(stock.id)"
        >
          复制到交易日
        </button>
        <button
          class="action-btn btn-copy-date"
          @click.stop="onCopyDate(stock.id)"
        >
          复制到日期
        </button>
        <button
          class="action-btn btn-delete"
          @click.stop="onDelete(stock.id)"
        >
          删除
        </button>
      </div>
      ...
      >
        <div
          v-for="record in soldRecordsReversed(stock)"
          :key="record.date"
      ...
      <div
        class="track-simple"
        @click="onTrackEdit(stock)"
      >
        <div class="track-simple-head">
          📌 追踪记录
        </div>
        <div
          v-if="trackItems(stock.track).length"
          class="track-scroll-container"
        >
          <div class="track-window content-display">
            <div
              v-for="(item, idx) in trackItems(stock.track)"
              :key="(item.date||'')+'|'+idx"
              class="track-item"
            >
