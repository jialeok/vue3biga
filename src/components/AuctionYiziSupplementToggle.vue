...
const hint = '打开：把「竞价一字」看板的一字板股票【按题材并入下方对应的题材组】，'
  + '并与该组原有股票【一起按十日涨幅排名】：序号 = 它在本题材组内的十日涨幅名次（补入行插到自己的排名位），'
  + '行外观与列表完全一致（只显示 序号 / 股票名称 / 标签 / 题材 四列），名称后带紫色「补」标。'
  + '股票名已在列表里的不再重复列出；题材在当日列表里没有对应分组的收在最下方「未并入」里。'
  + '点序号可展开该股近 5 日趋势。数据取自竞价一字看板自己的库，与早盘竞价的列表 / 排序互不影响；'
  + '关闭即恢复原样。';
</script>
