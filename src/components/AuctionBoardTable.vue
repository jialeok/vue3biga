...
<template>
  ...
  <!-- ★【题材单独开启 + 补竞价一字打开】→ 渲染「按题材融入」后的序列：
       每个题材组 = 该组原有行（原样渲染）+ 按十日涨幅合并排名插位的补入行（带「补」标）。
       组与组之间不插任何东西 → 视觉上就是「原列表每个题材组里多了几行带标行」。
       组【间】顺序已由 logic/auction/yizi-topic-order.js 按「一字权重（含补入的）降序」重排
       （⚠️ 只在开关打开时生效；开关一关本分支整体不渲染 → 组序与序号立刻回到既有原样）。 -->
