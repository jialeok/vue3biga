...
<template>
  ...
  >
    <div class="auction-toolbar">
      ...
      <div class="auction-toggle-item">
        ...
        <label class="auction-toggle-switch">
          <input
            type="checkbox"
            :checked="sortState2.byRatio"
            @change="toggleSort2('byRatio')"
          >
          <span class="auction-toggle-slider" />
        </label>
      </div>
      <div class="auction-toggle-item">
        ...
        <label class="auction-toggle-switch">
          <input
            type="checkbox"
            :checked="sortState2.byParallel"
            @change="toggleSort2('byParallel')"
          >
          <span class="auction-toggle-slider" />
        </label>
      </div>
    ...
    <div class="auction-toolbar-row2">
      <div class="auction-toggle-item">
        ...
        <label class="auction-toggle-switch">
          <input
            type="checkbox"
            :checked="sortState2.byJingYest"
            @change="toggleSort2('byJingYest')"
          >
          <span class="auction-toggle-slider" />
        </label>
      </div>
      <div class="auction-toggle-item">
        ...
        <label class="auction-toggle-switch">
          <input
            type="checkbox"
            :checked="sortState2.byJingYestRatio"
            @change="toggleSort2('byJingYestRatio')"
          >
          <span class="auction-toggle-slider" />
        </label>
      </div>
      <div class="auction-toggle-item">
        ...
        <label class="auction-toggle-switch">
          <input
            type="checkbox"
            :checked="sortState2.byThreeDayJingDie"
            @change="toggleSort2('byThreeDayJingDie')"
          >
          <span class="auction-toggle-slider" />
        </label>
      </div>
    ...
    <div class="auction-highratio-stat">
      <span style="font-weight:700;color:#dc2626;">竞昨数：{{ p2JingYestCount }}</span><span style="display:inline-block;width:28px;" />竞放量数：<span style="font-weight:700;">{{ p2HighRatioCount }}</span>
    </div>
    <div
      v-if="sortedTopicGroups.length === 0"
      class="auction-empty"
    >
      暂无题材分组数据
    </div>
    <div v-else>
      <div class="auction-header-row">
        <div
          class="auction-header-item auction-header-stock"
          style="flex:0 0 75px;padding-left:10px;"
        >
          股票名称
        </div>
        <div
          class="auction-header-item auction-header-change"
          style="flex:0 0 55px;"
        >
          涨幅
        </div>
        <div
          class="auction-header-item auction-header-volume"
          style="flex:1;text-align:left;padding-left:8px;"
        >
          题材
        </div>
        <div
          class="auction-header-item"
          style="flex:0 0 70px;cursor:pointer;"
          @click="toggleStrengthSort"
        >
          <span :class="{ 'strength-sort-active': isStrengthSortEnabled }">{{ isStrengthSortEnabled ? '▼强度' : '强度' }}</span>
        </div>
        <div
          class="auction-header-item auction-header-ratio"
          style="flex:0 0 50px;"
        >
          占比
        </div>
      ...
      <div
        v-for="group in sortedTopicGroups"
        :key="group.topic"
        class="auction-topic-group"
        :data-topic-group="group.topic"
      >
        <div
          v-if="canGroupExpand(group.topic)"
          class="auction-topic-expand-row"
          @click="toggleGroupExpand(group.topic)"
        >
          <span
            class="auction-topic-expand-arrow"
            :class="{ expanded: p2ExpandedTopics.has(group.topic) }"
          >▼</span>
        </div>
        <div class="auction-topic-header">
          ...
          <span
            v-if="group.topic !== '其它'"
            class="auction-topic-stars"
          >{{ getStarSymbols(group.starCount) }}</span>
          <span
            v-if="group.topic !== '其它' && group.strength !== null"
            class="auction-topic-strength"
          > 强度<span style="color:#ef4444;">{{ group.strength }}%</span></span>
          <span class="auction-topic-count">{{ group.stocks.length }}只</span>
        </div>
        <template
          v-for="stock in group.stocks"
          :key="group.topic + '-' + stock.stock"
        >
          <div
          ...
          >
            <div
              class="auction-topic-stock"
              :style="getStockStyle(stock.stock)"
            >
              {{ stock.stock || '-' }}
            </div>
            <div :class="getChangeClass(stock)">
              {{ getChangePctDisplay(stock) }}
            </div>
            <div
              class="auction-topic-name"
              :style="getTopicNameStyle()"
              @click.stop="openCoreTopicModal"
            >
              {{ getTopicsDisplay(stock) }}
            </div>
            <div class="auction-topic-ratio">
              {{ stock.ratio }}
            </div>
          </div>
          <div
          ...
          >
            <template v-if="p2TrendHistory[group.topic + '|' + stock.stock]">
              <div class="auction-daily-metrics">
                <template
                  v-for="m in dailyMetricsList(stock.stock)"
                  :key="m.label"
                >
              ...
              <div class="trend-chart-item">
                <div class="trend-chart-label trend-chart-label-with-stats">
                  <span>竞价量(万) 近5日</span>
                  <span
                    v-if="p2TrendHistory[group.topic + '|' + stock.stock].diff != null"
                    style="color:#2563eb; font-weight:600;"
                  >差值 {{ p2TrendHistory[group.topic + '|' + stock.stock].diff }}</span>
                  <span
                    v-if="p2TrendHistory[group.topic + '|' + stock.stock].jingRatio != null"
                    style="color:#6366f1; font-weight:600;"
                  >今/昨比 {{ p2TrendHistory[group.topic + '|' + stock.stock].jingRatio }}</span>
                </div>
                <TrendChart
                  :points="p2TrendHistory[group.topic + '|' + stock.stock].volume"
                  color="#6366f1"
                />
              </div>
              <div class="trend-chart-item">
                <div class="trend-chart-label trend-chart-label-with-stats">
                  ...
                  <span
                    v-if="p2TrendHistory[group.topic + '|' + stock.stock].yestRatio != null"
                    style="color:#10b981; font-weight:600;"
                  >昨/前比 {{ p2TrendHistory[group.topic + '|' + stock.stock].yestRatio }}</span>
                </div>
                <TrendChart
                  :points="p2TrendHistory[group.topic + '|' + stock.stock].yestVolume"
                  color="#10b981"
                />
              </div>
              <div
              ...
              >
                ...
                <TrendChart
                  :points="p2TrendHistory[group.topic + '|' + stock.stock].aucPctChg"
                  color="#f59e0b"
                  :percent="true"
                />
              </div>
              <div
                v-if="p2ChangePctHasData(group.topic, stock.stock)"
                class="trend-chart-item"
              >
                <div class="trend-chart-label">
                  涨幅(%) 近5日
                ...
                <TrendChart
                  :points="p2TrendHistory[group.topic + '|' + stock.stock].changePct"
                  color="#64748b"
                  :percent="true"
                />
              </div>
