...
- 🔴 **症状**：浏览器空白页，console `Uncaught ReferenceError: getAuctionData is not defined`（app-core 顶层 `_bindApi({...})` 注册 API 门面时，把迁到域模块的 `getAuctionData` 等当模块内局部变量引用）。次生 `Cannot read properties of undefined (reading '_s')` 为模块初始化失败的级联。
- 🔍 **根因（§16 反模式）**：域拆分时函数用 `export { X } from './domain/x.js'` 这种 **re-export** 迁出，但 re-export **不会在本模块创建局部绑定**。app-core.js 仅 re-export 未 import，导致 `_bindApi({...getAuctionData, getHotAuctionData, getStockHistoryTopics, getRankData, getTagTitlesData, getTodayJiwang, markJiwangDirty, replaceConceptFromPaste, importHotFromPaste, replaceHotConceptFromPaste, _openHotAuctionShield, _closeHotAuctionShield, getTodayAuction, getTodayGroupList, markAuctionDirty, patchAuctionFieldBatch, reconcileAuctionWatchlistFromLocalStorage, mergeAuctionDateRows, importAuctionFromPaste})` 在求值期引用未定义变量 → 整模块初始化失败 → 空白页。`vite build` 通过但**不报此错**（构建只校验 import/导出静态图，运行期才暴露）。
- ✅ **修复**：app-core.js 顶部补一组 `import`（与 date-helpers 的 import+export 写法一致），让所有 re-export 名成为本模块局部绑定；下方 re-export 行保留供外部调用点零破坏。循环依赖安全（域模块仅在函数体内调用 app-core 函数，顶层不调用）。
