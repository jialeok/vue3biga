        ...
        // 清理竞价变化行：封单家数只应出现在 9:25（time930）列，收盘列不应有值
        ...
        export async function pullBiddingFromTable() {
            ...
            window._biddingTableAvailable = true;
            return result;
        }
        ...
        export async function pullBiddingForDate(date) {
            ...
            if (window._biddingPushInFlight && window._biddingPushInFlight.has(date)) {
                window._dbgLog('[BIDDING-PULL] 跳过 ' + date + '，该日期正在推送中');
                return;
            }
            const sb = window.getSupabase();
            const { data, error } = await sb.from('bidding_data')
        ...
        // 把 time930_initial_modifiedAt / time930_modifiedAt 这类"修改时间"字段
        ...
        export async function pushBiddingToCloud(date) {
            ...
            window._justPushedBidding = true; // 标记自己推送，5秒内忽略 Realtime 通知
            setTimeout(function() { window._justPushedBidding = false; }, 5000);
            if (date) {
                window._biddingPushInFlight.add(date);
                window._biddingDirtyDates.add(date);
            }
            try {
                const sb = window.getSupabase();
                const biddingData = window.getBiddingData()[date] || [];
                if (biddingData.length === 0) return;
                const now = new Date().toISOString();
                const rows = biddingData.filter(function(r) { return r && r.name; }).map(function(item) {
                    window.sanitizeBiddingRow(item);
                    var row = {
                        date: date,
                        name: item.name,
                    ...
                    if (item.time930_initial_modifiedAt !== undefined && item.time930_initial_modifiedAt !== null) {
                        row.time930_initial_modifiedAt = window.toIsoTimestamp(item.time930_initial_modifiedAt);
                    }
                    if (item.time930_modifiedAt !== undefined && item.time930_modifiedAt !== null) {
                        row.time930_modifiedAt = window.toIsoTimestamp(item.time930_modifiedAt);
                    }
                    return row;
                });
                ...
                const hasAnyValue = rows.some(function(r) {
                    return (r.time915 && r.time915.trim() !== '') ||
                           ...
                           (r.time930 && r.time930.trim() !== '') ||
                           (r.change && r.change.trim() !== '') ||
                           (r.close && r.close.trim() !== '');
                });
                if (!hasAnyValue) {
                    window._dbgLog('[BIDDING-PUSH] 跳过 ' + date + ' 的空数据推送，避免覆盖云端已有数据');
                ...
                if (error) throw error;
                window._biddingTableAvailable = true; // 推送成功即证明表可用
                // 推送成功：该日期的本地编辑已同步，可从脏标记中移除
                if (date) window._biddingDirtyDates.delete(date);
                window._dbgLog('[BIDDING-PUSH] 成功推送 ' + date + ' 共 ' + rows.length + ' 行');
            } finally {
                if (date) window._biddingPushInFlight.delete(date);
            }
        }
        ...
        export async function syncBiddingDeletionsToCloud(date) {
            if (date) window._biddingPushInFlight.add(date);
            try {
                const sb = window.getSupabase();
                const localNames = new Set((window.getBiddingData()[date] || [])
                ...
                window._justPushedBidding = true; // 删除也会触发 Realtime，同样屏蔽
                setTimeout(function() { window._justPushedBidding = false; }, 5000);
                const { error: delErr } = await sb.from('bidding_data')
                    .delete().eq('date', date).in('name', toDelete);
                if (delErr) throw delErr;
                window._dbgLog('[BIDDING-DELETE] 成功删除 ' + date + ' 的 ' + toDelete.length + ' 个幽灵行: ' + toDelete.join(','));
            } finally {
                if (date) window._biddingPushInFlight.delete(date);
            }
        }
        ...
        export async function deleteBiddingFromCloud(date) {
            if (!date) return;
            window._justPushedBidding = true;
            setTimeout(function() { window._justPushedBidding = false; }, 5000);
            const sb = window.getSupabase();
        ...
        // （阶段四 Bug 3 修复：与 bidding 对齐，"清空当日数据"原本只 delete allData.auction[window.currentDate]，
        ...
        export async function deleteAuctionFromCloud(date) {
            ...
            try {
                const sb = window.getSupabase();
                let beforeCount = 0;
                if (window._auctionTableAvailable) {
                ...
                window._dbgLog('[AUCTION-DELETE] 准备删除 ' + date + ' 的云端行，删除前 ' + beforeCount + ' 条');
                ...
                window._dbgLog('[AUCTION-DELETE] 已删除 ' + date + ' 的 ' + beforeCount + ' 条云端行');
            } finally {
        ...
        export async function migrateBiddingToTable() {
            if (localStorage.getItem('_bidding_table_migrated') === '1') {
                window._biddingTableAvailable = true; // 已迁移过，标记表可用
                return;
            }
            ...
            let biddingData = {};
            try {
                const raw = localStorage.getItem(window._moduleKey('bidding'));
            ...
            if (!hadError) {
                ...
                try { localStorage.removeItem(window._moduleKey('bidding')); } catch (e) {}
        ...
        export function startBiddingRealtime() {
            ...
            try {
                const sb = window.getSupabase();
                window._biddingRealtimeChannel = sb
                    .channel('bidding_data_changes')
                    .on('postgres_changes', {
                        event: '*', schema: 'public', table: 'bidding_data'
                    }, function(payload) {
                        if (window._justPushedBidding) return; // 自己刚推的，忽略
                        const row = payload.new || payload.old;
                        if (!row || !row.date) return;
                        const changedDate = row.date;
                        // 拉取该日期的最新竞价数据并刷新看板
                        window.pullBiddingForDate(changedDate).then(function() {
                            if (changedDate === window.currentDate) window._emit('data:realtime-update', { boards: 'bidding' });
                        }).catch(function(e) { window._dbgLog('[AUCTION-ERR] Bidding Realtime 拉取失败 ' + (e && e.message || e)); });
                    })
                    .subscribe();
                console.log('Bidding Realtime 订阅已启动');
            } catch (e) { window._dbgLog('[AUCTION-ERR] Bidding Realtime 订阅失败 ' + (e && e.message || e)); }
        }

        export function stopBiddingRealtime() {
            if (window._biddingRealtimeChannel) {
                try { window.getSupabase().removeChannel(window._biddingRealtimeChannel); } catch(e) {}
                window._biddingRealtimeChannel = null;
            }
        }
