// numcat-proxy.js — 猫抓 numcat 接口代理（data/api/ 层）
// 从 app-core.js 抽离，纯数据层：不碰 DOM，只负责 HTTP 请求和错误处理

window.NUMCAT_PROXY_URL = 'https://tonqfgeyxnnwicjopshn.supabase.co/functions/v1/numcat-proxy';
...
export async function numcatApiPost(apiname, fields, params, pathOverride) {
    ...
    let resp;
    try {
        resp = await fetch(window.NUMCAT_PROXY_URL, {
            method: 'POST',
            headers: {
        ...
        window._dbgLog('[NUMCAT-NET-ERR] ' + apiname + ': ' + (netErr && netErr.message || netErr));
        throw new Error('代理请求失败（网络层）：' + (netErr && netErr.message || '未知错误') + '。请确认 numcat-proxy Edge Function 已部署且 NUMCAT_API_KEY 已设置');
    }
    if (!resp.ok) {
        let errBody = '';
        try { errBody = await resp.text(); } catch (e) {}
        window._dbgLog('[NUMCAT-HTTP-ERR] ' + apiname + ': HTTP ' + resp.status + ' ' + resp.statusText + ' body=' + errBody.slice(0, 200));
        if (resp.status === 404) {
            throw new Error('numcat-proxy Edge Function 未部署（HTTP 404）。请在 Supabase Dashboard 部署 numcat-proxy 并设置 NUMCAT_API_KEY');
        }
        if (resp.status === 401 || resp.status === 403) {
            throw new Error('numcat-proxy 鉴权失败（HTTP ' + resp.status + '）。请检查 window.SUPABASE_ANON_KEY 配置');
        }
        if (resp.status === 500) {
    ...
    try {
        ...
        window._dbgLog('[NUMCAT-JSON-ERR] ' + apiname + ': 响应非 JSON，body=' + rawBody.slice(0, 200));
