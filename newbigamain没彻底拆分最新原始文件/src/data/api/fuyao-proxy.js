...
window.FUYAO_PROXY_BASE = 'https://tonqfgeyxnnwicjopshn.supabase.co/functions/v1/fuyao-proxy';
window.FUYAO_DIRECT_BASE = 'https://fuyao.aicubes.cn';
window.LADDER_THSCODE = '883410.TI';

// 调用同花顺 fuyao-proxy（GET），返回 data 字段
export async function fuyaoApiGet(path, params) {
    const url = new URL(window.FUYAO_PROXY_BASE);
    ...
    try {
        ...
        window._dbgLog('[FUYAO-NET-ERR] ' + path + ': ' + (netErr && netErr.message || netErr));
        throw new Error('代理请求失败（网络层）：' + (netErr && netErr.message || '未知错误') + '。请确认 fuyao-proxy Edge Function 已部署到 Supabase');
    }
    if (!resp.ok) {
        let errBody = '';
        try { errBody = await resp.text(); } catch (e) {}
        window._dbgLog('[FUYAO-HTTP-ERR] ' + path + ': HTTP ' + resp.status + ' ' + resp.statusText + ' body=' + errBody.slice(0, 200));
        if (resp.status === 404) {
            throw new Error('fuyao-proxy Edge Function 未部署（HTTP 404）。请在 Supabase Dashboard 部署 fuyao-proxy');
        }
        if (resp.status === 401 || resp.status === 403) {
            throw new Error('fuyao-proxy 鉴权失败（HTTP ' + resp.status + '）。请检查 window.SUPABASE_ANON_KEY 配置');
        }
    ...
    try {
        ...
        window._dbgLog('[FUYAO-JSON-ERR] ' + path + ': 响应非 JSON，body=' + rawBody.slice(0, 200));
...
export async function fuyaoDirectGet(path, params, apiKey) {
    const url = new URL(window.FUYAO_DIRECT_BASE + path);
    for (const k in params) {
        if (params[k] !== undefined && params[k] !== null) {
            url.searchParams.set(k, params[k]);
        }
