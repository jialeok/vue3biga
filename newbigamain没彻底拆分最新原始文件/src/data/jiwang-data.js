        ...
        export async function pullJiwangFromTable() {
            ...
            if (duplicateDates) {
                ...
                window._dbgLog && window._dbgLog('pullJiwangFromTable: 发现重复日期行: ' + duplicateDates.join(', '));
            }
            window._jiwangTableAvailable = true;
            return result;
        }
        ...
        export async function pullJiwangForDate(date) {
            ...
            const pending = (window._jiwangDirtyDates && window._jiwangDirtyDates.has(date)) ||
                ...
                (window._justPushedJiwang && date === window.currentDate);
            if (pending) {
                window._dbgLog && window._dbgLog('pullJiwangForDate: ' + date + ' 有本地待推送编辑，跳过覆盖');
                return;
            }
            const sb = window.getSupabase();
            ...
            if (rows.length > 1) {
                ...
                window._dbgLog && window._dbgLog('pullJiwangForDate: ' + date + ' 发现 ' + rows.length + ' 条重复行');
            }
        ...
        export async function pushJiwangToCloud(date) {
            ...
            if (!item) {
                window._dbgLog('pushJiwangToCloud: ' + date + ' 本地无数据，跳过（可能已被清空）');
                return; // 本地这天没有数据（可能已被清空），交给 window.deleteJiwangFromCloud 处理
            }
            window._justPushedJiwang = true;
            setTimeout(function() { window._justPushedJiwang = false; }, 5000);
            const sb = window.getSupabase();
            const row = {
                date: date,
            ...
            window._dbgLog('pushJiwangToCloud: 开始 upsert ' + date);
            ...
            if (error) {
                window._dbgLog('pushJiwangToCloud: ' + date + ' upsert 失败: ' + (error.message || JSON.stringify(error)));
                throw error;
            }
            if (!upserted || upserted.length === 0) {
                const msg = 'upsert 未返回任何行，写入可能未真正生效（检查 jiwang_data.date 是否有唯一约束、以及 RLS 策略）';
                window._dbgLog('pushJiwangToCloud: ' + date + ' ' + msg);
                throw new Error(msg);
            }
            if (upserted.length > 1) {
                window._dbgLog('pushJiwangToCloud: ' + date + ' upsert 返回了 ' + upserted.length + ' 行，说明 date 列缺少唯一约束，onConflict 未生效，产生了重复行——请在 Supabase 给 jiwang_data.date 加唯一约束');
            }
            window._dbgLog('pushJiwangToCloud: ' + date + ' upsert 成功');
            window._jiwangTableAvailable = true;
        }
        ...
        export function scheduleJiwangPush(date) {
            if (!date) return;
            window._dbgLog('scheduleJiwangPush: 排队 ' + date + '（2秒防抖）');
            clearTimeout(window._jiwangPushTimers[date]);
            window._jiwangPushTimers[date] = setTimeout(function() {
                delete window._jiwangPushTimers[date];
                window.pushJiwangToCloud(date).catch(function(e) {
                    console.warn('window.pushJiwangToCloud 失败:', e);
                    const detail = (e && (e.message || e.details || e.hint || e.code)) || String(e);
                    window._dbgLog('scheduleJiwangPush: ' + date + ' 推送失败: ' + detail);
                    window._emit('ui:toast', { type: 'warning', msg: '⚠️ 记忘看板云端同步失败，本次修改未保存！原因: ' + detail, duration: 10000 });
                });
            }, 2000);
        }
        ...
        export function pushJiwangNow(date, successMsg) {
            ...
            clearTimeout(window._jiwangPushTimers[date]);
            delete window._jiwangPushTimers[date];
            window._dbgLog('pushJiwangNow: 立即推送 ' + date);
            window.pushJiwangToCloud(date).then(function() {
                if (successMsg) window._emit('ui:toast', { type: 'success', msg: successMsg });
            }).catch(function(e) {
                console.warn('window.pushJiwangNow 失败:', e);
                const detail = (e && (e.message || e.details || e.hint || e.code)) || String(e);
                window._dbgLog('pushJiwangNow: ' + date + ' 推送失败: ' + detail);
                window._emit('ui:toast', { type: 'warning', msg: '⚠️ 记忘看板云端同步失败，本次修改未保存！原因: ' + detail, duration: 10000 });
            });
        }
        ...
        export async function migrateJiwangToTable() {
            ...
            let jiwangData = {};
            try {
                const raw = localStorage.getItem(window._moduleKey('jiwang'));
            ...
            try {
                ...
                try { localStorage.removeItem(window._moduleKey('jiwang')); } catch (e) {}
        ...
        export function startJiwangRealtime() {
            ...
            try {
                const sb = window.getSupabase();
                window._jiwangRealtimeChannel = sb
                    .channel('jiwang_data_changes')
                    .on('postgres_changes', {
                        event: '*', schema: 'public', table: 'jiwang_data'
                    }, function(payload) {
                        if (window._justPushedJiwang) { window._dbgLog('JiwangRealtime: 收到变化但是自己刚推送的，忽略'); return; } // 自己刚推的，忽略
                        const row = payload.new || payload.old;
                        if (!row || !row.date) return;
                        const changedDate = row.date;
                        window._dbgLog('JiwangRealtime: 收到 ' + changedDate + ' 变化（他端推送），拉取该日数据');
                        window.pullJiwangForDate(changedDate).then(function() {
                            if (changedDate === window.currentDate) {
                                window._emit('data:realtime-update', { boards: 'jiwang' });
                                window._emit('data:realtime-update', { boards: 'marketStage' });
                            }
                        }).catch(function(e) { console.warn('Jiwang Realtime 拉取失败:', e); window._dbgLog('JiwangRealtime: 拉取失败: ' + (e && e.message)); });
                    })
                    .subscribe();
        ...
        export function stopJiwangRealtime() {
            if (window._jiwangRealtimeChannel) {
                try { window.getSupabase().removeChannel(window._jiwangRealtimeChannel); } catch(e) {}
                window._jiwangRealtimeChannel = null;
            }
        }
