...
--
--   ⚠️【重要】本 cron 只负责「当天」——它今天 09:25 之前还不存在的话，
--     今天 09:25 就已经过去了，**第一个自动触发点只能等到下一个交易日**。
--     部署当天看板空白 ≠ 部署出错（已实测：函数/端点/key/表全对，只是还没到触发点）。
--     历史日期请用回填脚本补（幂等、自动跳过已有日期、命中额度立即止损）：
--       HTTPS_PROXY=http://127.0.0.1:7897 NODE_USE_ENV_PROXY=1 \
--         node db/yizi-backfill.mjs --run --days=30 --max=7 --gap=22
--     额度约束：猫抓免费档每日 10 次（按 key 计，北京 0 点重置），本 cron 每日占 1 次。
--
-- ════════════════════════════════════════════════════════════════════════════
